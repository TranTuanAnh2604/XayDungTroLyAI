...
export default function AiInsightsCard({ report, skeleton = false }: AiInsightsCardProps) {
  ...
  const hasEvaluation = !!report.ai_Evaluation?.trim();
  ...
  return (
    <AppGlassCard variant="ai" padding={16}>
      ...
      {hasEvaluation ? (
        ...
        <Text style={styles.empty}>Chưa có phân tích AI cho tuần này.</Text>
      )}

      {hasEvaluation && report.ai_Evaluation.length > 120 && (
